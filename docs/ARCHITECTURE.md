...
`NFR-1.` Решение должно быть реализовано на языке Java без использования Spring, чтобы не усложнять архитектуру инфраструктурным кодом.
...
### 3.1. Почему peer-to-peer

По условию задания узлы должны взаимодействовать напрямую. Поэтому каждый экземпляр приложения поднимает собственный сервер и, при необходимости, устанавливает исходящее подключение к другому узлу. Центральный координатор, брокер или база сообщений не используются.
...
### 3.3. Почему Java без Spring

Выбор Java обоснован тем, что:

- язык подходит для сетевого и многопоточного программирования;
- существуют зрелые библиотеки для gRPC и тестирования;
- код удобно структурировать по пакетам и слоям.

Отказ от Spring обоснован тем, что:

- для консольного P2P-чата Spring не дает критической пользы;
- внедрение полноценного Spring-контейнера усложнит учебный проект;
- без Spring легче показать собственную архитектуру, а не каркас фреймворка.
...
Причина выбора минимального контракта: задание требует только отправку текстовых сообщений между двумя узлами. На данном этапе этого достаточно и не создает ненужной сложности.
...
- модульные тесты парсинга CLI-аргументов;
- модульные тесты `ProtoMessageMapper`;
- модульные тесты `ChatService`;
- интеграционный тест запуска двух peer-узлов и проверки доставки сообщения;
- негативные тесты на невалидные аргументы и ошибку подключения.

Такой набор покрывает основные риски: некорректный запуск, нарушение контракта сериализации и сбои в базовом сетевом сценарии.

## 13. Декомпозиция задач между двумя участниками
...
- вычитка документации и чеклиста перед сдачей.

## 14. Риски и ограничения

- Чат ориентирован на взаимодействие двух peer-узлов и не проектируется как многопользовательская система.
- При недоступности удаленного peer отправка сообщения невозможна до восстановления соединения.
- История сообщений в базовой версии хранится только в памяти процесса и не восстанавливается после перезапуска.
- GUI не входит в обязательную часть решения, так как в задании требуется консольный интерфейс.

## 15. Вывод

На данный документ можно опираться как на основу проектирования и реализации. Он покрывает требования задания, фиксирует выбранные технологии, описывает структуру решения и помогает заранее разделить ответственность между участниками команды без архитектурной избыточности.
